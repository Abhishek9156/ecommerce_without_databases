import 'package:e_commerce/consts/consts.dart';

const socialIconList = [icFacebookLogo, icGoogleLogo, icTwitterLogo];

const sliderList = [imgSlider1, imgSlider2, imgSlider3, imgSlider4];
const secondsliderList = [imgSs1, imgSs2, imgSs3, imgSs4];

const featuredImage1 = [imgS1, imgS2, imgS3, imgS4];
const featuredImage2 = [imgS4, imgS5, imgS6];
const featuredTitle1 = [womenDress, girlsDress, girlsWatches];
const featuredTitle2 = [boysGlass, mobilePhone, tShirts];
const featureProductList = [imgP1, imgP2, imgP3, imgP4, imgP5, imgP6];

const categoryList = [
  woMenClothing,
  menClothingFashion,
  compAccess,
  autoMobile,
  kidToy,
  sports,
  jewelery,
  cellPhone,
  furniture
];
const categoryImages= [
  imgFc1,
  imgFc2,
  imgFc3,
  imgFc4,
  imgFc5,
  imgFc6,
  imgFc7,
  imgFc8,
  imgFc9
];

const itemDetailButtonList=[
video,
reviews,
sellerpolicy,
returnpolicy,
supportpolicy
];

const profileButtonlist=[wishlist,orders, message];
const profileButtonIcons=[icOrder,icOrder,icMessages];